  ...
  try {
    ...
    if (!supabaseAdmin) {
      ...
      return NextResponse.json(
        { message: 'Database not configured - property saved to blockchain only' },
        { status: 200 }
      )
    }
    ...
    if (!validPropertyTypes.includes(property_type)) {
      logger.error('Invalid property_type', { property_type, valid: validPropertyTypes })
      return NextResponse.json(
    ...
    if (!validStatuses.includes(propertyStatus)) {
      logger.error('Invalid status', { status: propertyStatus, valid: validStatuses })
      return NextResponse.json(
    ...
    if (userError || !userData) {
      ...
      logger.info('Created basic user entry for seller', { wallet: normalizedAddress })
    }
    ...
    if (error) {
      logger.error('Failed to save property to database', { error, token_id, property_type })
      throw error
    }
