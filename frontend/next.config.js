...
const nextConfig = {
  ...
  // Optimize on-demand compilation - keep pages in memory longer
  // This prevents recompilation on every page visit
  onDemandEntries: {
    // Keep pages in memory for 5 minutes (instead of 25 seconds)
    // This means once a page is compiled, it stays compiled for 5 minutes
    maxInactiveAge: 5 * 60 * 1000, // 5 minutes (300,000ms)
    // Keep more pages in buffer to avoid recompilation
    // Higher number = more pages stay compiled in memory
    pagesBufferLength: 10, // Increased from 2 to 10
  },

  // Vercel uses its own build system, so we don't set output: 'standalone'
  // But we can enable outputFileTracing for better optimization
  ...(process.env.VERCEL && {
